# AI Agent Instructions — IronRoot CLI App Template
...
| | |
|---|---|
| Kind | Command-line tool (`ironroot-cli` + `ironroot-core`) |
| Package | `ironroot-cli-app` |
| Roadmap phase | Phase 3 — Templates (Web / CLI / GUI) |
| Standalone | Yes — not a workspace member; build from this directory |

A CLI is not exempt from §5. Command arguments, environment variables, file paths, and stdin
are all untrusted input: bound their length, reject what you do not recognise, and never
interpolate them into a query or a shell command. Exit codes and stderr are the CLI's error
surface — an unhandled panic is a bug, not an error path.
...
|---|---|---|---|---|
| Library | `src/lib.rs` | `banner` and `render` — every decision the CLI makes | — | `mod tests` at the bottom of the file |
| Binary | `src/main.rs` | collects `argv`, calls `render`, prints the result | library | none, by design — keep it that way |
...
cargo test --lib arguments  # the closest tests, filtered by name
```
