# CLAUDE.md — IronRoot Desktop App Template
...
The client is untrusted: enforce every check that matters server-side too. Keep UI callbacks
thin so business logic in `src/` stays unit- and BDD-testable. Credentials belong in the OS
keychain, never in a file beside the binary.
