//! Behaviour tests for `ironroot-desktop-app` — AGENTS.md §4.2.
//!
...
//!
//! Note what this buys a desktop application: the scenarios below run without a
//! window, a display server, or an event loop, because the logic they exercise
//! lives in `src/` rather than in a UI callback. Keep new logic there for the
//! same reason.
//!
...
fn render_banner(world: &mut BannerWorld) {
    world.rendered = Some(ironroot_desktop_app::banner());
}
